...
class MainTest {
	...
	/*
	 * @Test public void rulef092Executor500500FTest() { String CLLEGA; String
	 ...
	 * litr2.next(); ListIterator<BigDecimal> litr3 = CLSPESSValues.listIterator();
	 * while (litr3.hasNext()) { CLSPESS = litr3.next();
	 * assertEquals(Main.testRulef092ExecutorStandard(CLSPESS,CLLARG,SLTOLLA),
	 ...
	 * Main.Rulef089(CLSPESS, CLLEGA, CLSTATF, SLBPTE, SLBP, CLFINI, CLASSE, CLLARG,
	 * CLLUNG, CLRIVE, CLTOLLE, DLDEST, DLLAV, DLLUBRI, MLSTATF, PLACKG, PMETC,
	 * QSPEC, SLTOLLA, DLLATOLP, DLLATOLN, SLLATOLP, SLLATOLN)); } }
	 */
	...
	public void ruleCLSTATFTest() {
		...
		for (ICaratteristica caratteristica : returnCases) {
			...
			ruleBookCLLARG.getResult().ifPresent(result -> {
				System.out.println("Vincolo per CLSTATF con valore " + caratteristica.getCLSTATF().toString()
						+ " Valore per CLLEGA " + caratteristica.getCLLEGA().toString() + "; per SLBPTE "
						+ caratteristica.getSLBPTE().toString() + "; per SLBP " + caratteristica.getSLBP().toString()
						+ " validato " + result.toString());
				assertEquals(result.getValue(),
						Main.testCLSTATFRuleStandard(caratteristica.getCLLEGA().toString(),
								caratteristica.getCLSTATF().toString(), caratteristica.getSLBPTE().toString(),
	...
	public void ruleMLSTATFTest() {
		...
		for (ICaratteristica caratteristica : returnCases) {
			...
			ruleBookMLSTATF.getResult().ifPresent(result -> {
				System.out.println("Vincolo per CLSTATF con valore " + caratteristica.getCLSTATF().toString()
						+ " Valore per CLLEGA " + caratteristica.getCLLEGA().toString() + "; per CLLARG "
						+ caratteristica.getCLLARG().toString() + "; per SLBP " + caratteristica.getSLBP().toString()
						+ "; per CLLUNG " + caratteristica.getCLLUNG().toString() + "; per CLSPESS "
				...
				assertEquals(result.getValue(),
						Main.testMLSTATFRuleStandard(caratteristica.getCLLEGA().toString(),
								caratteristica.getCLSTATF().toString(), caratteristica.getCLLARG().toString(),
