...
public class Main2 {
	...
	public static void main(String[] args) {
		/*
		RuleBook<Boolean> caratteristicheRuleBook = RuleBookBuilder.create(CaratteristicheRuleBook.class)
			    ...
			    facts.setValue("CLSTATF", "H3");
			    facts.setValue("CLLEGA", "1A");
			    ...
			    System.out.println("Stato Rule1 " + auditor.getRuleStatus("Rule1")); //prints EXECUTED
			    System.out.println("Stato Rule2 " + auditor.getRuleStatus("Rule2")); //prints SKIPPED
			    System.out.println("Stato Rule3 " + auditor.getRuleStatus("Rule3")); //prints SKIPPED
			    
			    caratteristicheRuleBook.getResult().ifPresent(result -> System.out.println("stato fisico H00 ammesso per lega 1A : " + result));
		*/
...
*/
		//Predicate<String> libtest1 = s -> s.startsWith("cart.test.library.subrules1");
...
/*		
		...
		System.out.println("Tra i seguenti valori");
		ListIterator<String> litr = CLLEGAValues.listIterator();
		while(litr.hasNext()){
			System.out.println(litr.next());
	    }
		String n = c.readLine();    
		System.out.println("Hai selezionato " + n);    
		CLLEGA = n;
*/
//		CLLEGA = "3A";
...
/*		
		...
		System.out.println("Tra i seguenti valori");
		litr = CLSTATFValues.listIterator();
		while(litr.hasNext()){
			System.out.println(litr.next());
	    }
		n = c.readLine();    
...
//	    CaratteristicaBean applicant1 = new CaratteristicaBean(new BigDecimal(650), "3A", "H14", "B07187", "B07187");
	    ...
	    ruleBook.getResult().ifPresent(result -> System.out.println("Vincolo per Caratteristica stato fisico " + clstatfRis + " validato " + result));
...
//		noLoop();
	...
	private static boolean testVincolo(String CLLEGA, String CLSTATF, String SLBPTE, String SLBP) {
		...
		if( CLSTATF.equalsIgnoreCase("t04") || CLSTATF.equalsIgnoreCase("t06")
		|| CLSTATF.equalsIgnoreCase("t4p"))
		...
		if(CLLEGA.substring(0,1).equalsIgnoreCase("6")) {
			if(CLSTATF.substring(0,2).equalsIgnoreCase("h3"))
				validate = Boolean.FALSE;
			if(CLSTATF.equalsIgnoreCase("t04") || CLSTATF.equalsIgnoreCase("t06") ||
				   CLSTATF.equalsIgnoreCase("t4p") || CLSTATF.equalsIgnoreCase("h00") )
				validate = Boolean.TRUE;
		}
	...
	private static void noLoop() {
	    ...
	    ruleBook.getResult().ifPresent(result -> System.out.println("Vincolo per Caratteristica stato fisico "  + " validato " + result));
