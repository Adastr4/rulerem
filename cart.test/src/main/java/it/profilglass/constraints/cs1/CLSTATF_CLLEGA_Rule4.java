...
@Rule(order = 1, name="Rule4") // order specifies the order the rule should execute in; if not specified, any
					// order may be used
...
//		System.out.println("executing rule 5 package cs1");
		return caratteristiche.stream()
				...
				&& (caratteristiche.stream()
						.anyMatch(caratteristica -> caratteristica.getCLSTATF().equalsIgnoreCase("t04"))
						|| caratteristiche.stream()
						...
						|| caratteristiche.stream()
								.anyMatch(caratteristica -> caratteristica.getCLSTATF().equalsIgnoreCase("h00")));
	}
