...
public class RuleSBADEFIMB_40 extends it.profilglass.constraint.bav.SBADEFIMB.sub1.SubRuleSBADEFIMB_0 {
	...
	{		
		...
		return super.when()
			...
			&&
			   caratteristiche.stream().filter(caratteristica -> "SBADEFIMB".equals(caratteristica.getCaratteristicaId())).findAny().get().getSelectedValue().substring(1,6).equalsIgnoreCase("N003C");
