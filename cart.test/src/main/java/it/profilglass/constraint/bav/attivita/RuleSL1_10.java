...
@Rule(order = 10, name = "ruleSL1_1")

public class RuleSL1_10 extends it.profilglass.constraint.bav.attivita.subrules.SubRuleSL_2{
	@Given
	...
	public boolean when()
	{
		...
		return super.when()
			...
			&& caratteristiche.stream().filter(caratteristica -> "QLLAVDET".equals(caratteristica.getCaratteristica())).findAny().get().getSelectedValue().getOpzione().equalsIgnoreCase("H01");
	}

	@Then
	public RuleState then()
	{
		result.add(ReadDB.getAttivitaFromId("SL1"));
