...
public class RuleQLLAVDET_20 extends it.profilglass.constraint.bav.QLLAVDET.sub1.RuleQLLAVDET_20 {
	...
	{
		return super.when()
			&& caratteristiche.stream().filter(caratteristica -> "QLLAVDET".equals(caratteristica.getCaratteristicaId())).findAny().get().getSelectedValue().getOpzione().substring(1,2).equalsIgnoreCase("1");
	}
